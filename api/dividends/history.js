...
// Recent dividend events for display (last 12, with EUR total amount per payment)
...
// Estimate next payment amount in EUR based on the most recent payment in the same calendar month.
// Falls back to the simple average of the last 4 payments if no month match is found.
async function estimateNextPaymentEur(divs, nextMonth, gbpRate) {
  if (!divs.length || nextMonth == null) return null;
  // Look for the most recent payment in nextMonth
  const inMonth = divs
    .filter(d => new Date(d.date + 'T12:00:00Z').getMonth() === nextMonth)
    .sort((a, b) => b.date.localeCompare(a.date));
  if (inMonth.length > 0) {
    return toEur(inMonth[0].amount, inMonth[0].currency, gbpRate);
  }
  // Fallback: average of last 4 payments
  const last4 = divs.slice(-4);
  if (!last4.length) return null;
  let total = 0;
  for (const d of last4) total += await toEur(d.amount, d.currency, gbpRate);
  return total / last4.length;
}

// Calculate monthly average EUR dividend for the current calendar year
async function monthlyAvgCurrentYear(divs, gbpRate) {
  const year = new Date().getFullYear().toString();
  const thisYear = divs.filter(d => d.date.startsWith(year));
  if (!thisYear.length) return null;
  let total = 0;
  for (const d of thisYear) total += await toEur(d.amount, d.currency, gbpRate);
  // Divide by the number of distinct months that have already paid
  const months = new Set(thisYear.map(d => d.date.slice(0, 7)));
  ...
  for (const isin of isins) {
    ...
    try {
      ...
      results.push({
        ...
        monthlyAvgEur,             // average monthly dividend income in EUR for this position
        nextPaymentAmountEur,      // estimated EUR amount for the next dividend payment
