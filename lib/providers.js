...
const { fetchJustETFQuote } = require('./justetf');
...
async function fetchFromYahoo(isin, mapping) {
  try {
    ...
    const quote = Number.isFinite(result.price) && result.price > 0
      ? { close: result.price, price: result.price, currency: result.currency || mapping?.nativeCurrency || null } // #2: no dejar divisa nula
      : null;
...
async function fetchFromFundNav(isin, mapping = null) {
  try {
    const knownSecId = mapping?.morningstarSecId || null;
    const result = await getNavHistory(isin, knownSecId);
    const points = result?.points || [];
    if (!points.length) throw new Error('No NAV data from Morningstar/Leeway');
    const lastPt = points[points.length - 1];
    // Attach nativeCurrency from providers.json so buildAssets can convert to EUR when needed
    const nativeCurrency = mapping?.nativeCurrency || null;
    const quote = { close: lastPt.close, price: lastPt.close, ...(nativeCurrency ? { currency: nativeCurrency } : {}) };
...
async function fetchFromEodhd(isin, mapping) {
  try {
    ...
    // Si el proveedor no indica divisa, usa la divisa nativa de providers.json en vez de dejarla
    // nula (el cliente trataría una divisa desconocida como EUR → precio/beneficio mal escalados). #2
    const _eodCcy = quoteRes?.data?.currency || mapping?.nativeCurrency || null;
    const quote = quoteRes?.data ? { close: Number(quoteRes.data.close), price: Number(quoteRes.data.close), currency: _eodCcy } : (points.length > 0 ? { close: points[points.length - 1].close, price: points[points.length - 1].close, ...(mapping?.nativeCurrency ? { currency: mapping.nativeCurrency } : {}) } : null);
    const status = classifyCoverage(points, quote);
    return {
...
// ── JustETF (fallback for ETFs/ETCs not covered by Yahoo) ─────────────────────
async function fetchFromJustETF(isin) {
  try {
    const result = await fetchJustETFQuote(isin, 'EUR');
    if (!result || !Number.isFinite(result.price) || result.price <= 0) throw new Error('No price');
    const quote = { close: result.price, price: result.price, currency: result.currency, date: result.date };
    return {
      provider: 'justetf', symbol: isin, resolvedAs: 'isin',
      quote, history: [],
      coverage: { quote: true, history: false, status: 'PARTIAL', note: `JustETF: ${result.price} ${result.currency} (${result.date || 'n/a'})` }
    };
  } catch (err) {
    return {
      provider: 'justetf', symbol: isin, resolvedAs: null, quote: null, history: [],
      coverage: { quote: false, history: false, status: 'MISSING', note: `JustETF: ${err.message}` }
    };
  }
}
...
async function resolveFromCache(isin) {
  ...
  if (c) return c;
  try {
    // Always return Supabase history when it exists — even if the last data point is "stale".
    // Weekend gaps (e.g. Friday → Monday = 3 days) would falsely trigger live Yahoo fetches,
    // and Yahoo prices for UCITS funds are unreliable (stale quotes, wrong symbols).
    // Live refreshes happen explicitly in _bgRefreshPrices via refresh=1 (skipCache=true),
    // which bypasses this function entirely.  Removing the dbIsStale guard here stops the
    // value from bouncing between 84k and 100k on every page reload.
    const rows = await dbGetHistory(isin);
    if (rows && rows.length > 30) {
      // #2: el histórico cacheado NO guarda divisa → adjúntala desde el catálogo (providers.json).
      // Sin esto, un activo en USD servido desde caché llegaba al cliente sin divisa y se trataba como EUR.
      const _mapping = getAssetByIsin(isin);
      const _nc = _mapping?.nativeCurrency || null;
      // Marca el quote como "stale" si el último dato cacheado es de hace >5 días. El cliente
      // (coverageEurPrice) ignora el precio rancio para activos del seed y usa el precio del seed /
      // refresco vivo, en vez de mostrar un NAV congelado de hace meses. El histórico se sigue
      // devolviendo intacto (los gráficos lo necesitan) — esto NO dispara fetches en vivo.
      const _lastRow = rows[rows.length - 1];
      const _ageDays = _lastRow?.date ? (Date.now() - new Date(_lastRow.date).getTime()) / 86400000 : 9999;
      const _stale = _ageDays > 5;
      const quote = { close: _lastRow.close, price: _lastRow.close, date: _lastRow.date, ...(_stale ? { stale: true } : {}), ...(_nc ? { currency: _nc } : {}) };
...
async function resolveAssetData(isin, { skipCache = false, cacheOnly = false } = {}) {
  ...
  if (isFund(mapping)) {
    const navData = await fetchFromFundNav(isin, mapping);
  ...
  // 2. ETFs/stocks/unknown: Yahoo first (works via ISIN direct or yahooSymbol)
  const yahooData = await fetchFromYahoo(isin, mapping);
  ...
  // 3. No mapping → try Morningstar + JustETF in parallel, pick best
  if (!mapping) {
    const [navData, justEtfData] = await Promise.all([
      fetchFromFundNav(isin).catch(() => ({ coverage: { status: 'MISSING' } })),
      fetchFromJustETF(isin).catch(() => ({ coverage: { status: 'MISSING' } })),
    ]);
    // Pick best: prefer OK > PARTIAL > MISSING; prefer Morningstar (has history) over JustETF (quote only)
    let data = yahooData;
    if (navData.coverage.status === 'OK') data = navData;
    else if (justEtfData.coverage.status !== 'MISSING' && data.coverage.status === 'MISSING') data = justEtfData;
    else if (navData.coverage.status !== 'MISSING') data = navData;

    if (data.coverage.status !== 'MISSING') {
      const out = { isin, position, mapping: null, data, error: null };
      l1Set(isin, out);
      return out;
    }
    return { isin, position, mapping: null, data: null, error: 'Unknown asset — no data from Yahoo, Morningstar or JustETF' };
  }
  ...
  // 5. JustETF fallback — for ETFs/ETCs still MISSING after Yahoo + alt provider
  if (data.coverage.status === 'MISSING') {
    const justEtfData = await fetchFromJustETF(isin);
    if (justEtfData.coverage.status !== 'MISSING') data = justEtfData;
  }

  // 6. Universal Morningstar NAV fallback — last resort for any ISIN still MISSING
  if (data.coverage.status === 'MISSING') {
    const navData = await fetchFromFundNav(isin, mapping);
    if (navData.coverage.status !== 'MISSING') data = navData;
  }
