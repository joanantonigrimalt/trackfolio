  ...
  try {
    ...
    if (rows && rows.length > 30) {
      ...
      const out = {
        ...
        // stale-while-revalidate: always return cached history so the chart renders,
        // but mark as supabase_stale so coverage.js knows to trigger a background refresh
        data: {
          provider: stale ? 'supabase_stale' : 'supabase_cache',
          history: rows, quote,
          coverage: { status, note: `Supabase${stale ? ' (stale)' : ''}: ${rows.length} pts` }
        },
        error: null,
      };
