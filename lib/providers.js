...
async function resolveAssetData(isin, { skipCache = false, cacheOnly = false } = {}) {
  ...
  // 3. No mapping → try Morningstar NAV fallback, then best of what we have
  if (!mapping) {
    let data = yahooData;
    ...
    return { isin, position, mapping: null, data: null, error: 'Unknown asset — no data from Yahoo or Morningstar' };
  }
  ...
  // 5. Universal Morningstar NAV fallback — for any ISIN still MISSING (ETFs, unknowns, etc.)
  if (data.coverage.status === 'MISSING') {
    const navData = await fetchFromFundNav(isin);
    if (navData.coverage.status !== 'MISSING') data = navData;
  }
