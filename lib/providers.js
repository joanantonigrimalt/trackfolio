...
async function fetchFromEodhd(isin, mapping) {
  try {
    ...
    const quote = quoteRes?.data ? { close: Number(quoteRes.data.close), price: Number(quoteRes.data.close), currency: quoteRes.data.currency || null } : (points.length > 0 ? { close: points[points.length - 1].close, price: points[points.length - 1].close } : null);
    const status = classifyCoverage(points, quote);
    return {
