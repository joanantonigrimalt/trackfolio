...
async function fetchFromFundNav(isin) {
  try {
    const { points, fromDb } = await withDbCache(isin, 'fundnav', () => getNavHistory(isin).then(r => r?.points || null));
    const quote = points.length > 0 ? { close: points[points.length - 1].close, price: points[points.length - 1].close } : null;
    const status = classifyCoverage(points, quote);
    return {
      provider: 'fundnav', symbol: isin, resolvedAs: fromDb ? 'db_cache' : 'morningstar_or_leeway',
      quote, history: points,
      coverage: { quote: !!quote, history: points.length >= 60, status, note: `fundNav: ${points.length} pts${fromDb ? ' (cached)' : ''}` }
    };
  } catch (err) {
    return {
...
async function resolveAssetData(isin, { skipCache = false } = {}) {
  // L1 check
  if (!skipCache) { const c = l1Get(isin); if (c) return c; }

  const mapping = getAssetByIsin(isin);
  const position = getSeedPosition(isin);

  // 1. Always try Yahoo first (works for ETFs via exchange tickers AND funds via 0P*.F symbols)
  ...
  // 2. No mapping → return best we have
  if (!mapping) {
  ...
  // 3. Try recommended provider (eodhd / twelvedata / fmp) — mainly for assets where Yahoo gives PARTIAL
  ...
  // 4. If still MISSING and it's a fund → try fundNav (Morningstar/Leeway) as last resort
  if (data.coverage.status === 'MISSING' && isFund(mapping)) {
    const navData = await fetchFromFundNav(isin);
    if (navData.coverage.status !== 'MISSING') data = navData;
    else data = fundSourceFallback(mapping);
  }
...
module.exports = { getAssetByIsin, getSeedPosition, resolveAssetData, classifyCoverage };
