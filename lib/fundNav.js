...
async function mstarHistory(secId, startDate = '2021-01-01') {
  ...
  const url = `https://tools.morningstar.es/api/rest.svc/timeseries_price/${MSTAR_TOKEN}?id=${secId}&idtype=Morningstar&frequency=daily&startDate=${startDate}&endDate=${today}&outputType=JSON`;
  const res = await fetch(url, {
...
async function navFromMorningstar(isin, knownSecId = null) {
  const secId = knownSecId || await mstarSearch(isin);
...
async function getNavHistory(isin, knownSecId = null) {
  const key = `nav:${isin}`;
  const cached = cacheGet(key);
  if (cached) return cached;

  // 1. Morningstar (use knownSecId directly if provided, skip ISIN search)
  try {
    const result = await navFromMorningstar(isin, knownSecId);
...
// Get NAV history, optionally bypassing ISIN search with a known Morningstar secId
// Usage: getNavHistory('LU0084617165', 'F0GBR04AVQ') — skips search, fetches directly
