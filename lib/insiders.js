...
async function fetchInsidersForSymbol(symbol) {
  ...
  // L1 in-memory (warm instance) — skip empty cached results
  const l1 = getCache(cacheKey);
  if (l1 && l1.length > 0) return { data: l1, source: 'l1' };

  // L2 Supabase (shared across cold starts) — skip empty cached results
  const l2 = await dbGetInsiders(symbol);
  if (l2 && l2.length > 0) { setCache(cacheKey, l2, TTL); return { data: l2, source: 'l2' }; }
