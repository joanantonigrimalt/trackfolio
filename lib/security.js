...
function validateIsins(raw) {
  ...
  const all = String(raw).split(',').map(s => s.trim().toUpperCase()).filter(Boolean);
  if (all.length === 0) return { isins: [], error: 'No ISINs provided' };
  if (all.length > 50) return { isins: [], error: 'Maximum 50 ISINs per request' };
  // Filter to valid ISINs only — non-ISIN symbols (e.g. custom assets) are silently skipped
  const isins = all.filter(i => validateIsin(i));
  if (isins.length === 0) return { isins: [], error: 'No valid ISINs provided' };
