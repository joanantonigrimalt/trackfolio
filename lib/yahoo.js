  ...
  for (const url of urls) {
    try {
      const response = await fetch(url, {
        ...
        signal: AbortSignal.timeout(5000), // 5s per Yahoo call — prevents hanging on Vercel
      });
...
async function yahooSearch(query) {
  ...
  const response = await fetch(url, {
    ...
    signal: AbortSignal.timeout(5000), // 5s timeout — prevents hanging on Vercel
  });
  const data = await response.json();
...
async function yahooByIsin(isin, fallbackSymbol = null, range = '5y') {
  ...
  // 2. Known fallback symbol — accept even with sparse history as long as there is a valid price
  if (fallbackSymbol) {
    try {
      const result = await yahooChart(fallbackSymbol, range);
      if (result.points.length > 10 || (Number.isFinite(result.price) && result.price > 0)) {
        return { ...result, resolvedAs: 'yahoo_symbol' };
      }
    } catch (_) { /* try next */ }
  }
